...
static void format_end_time_string(time_t end_timestamp, const char* clock_type, char* buffer) {
...
static void draw_progress_bar(int remaining, int total, const char* end_time_str) {
...
static void update_display(int remaining, int total, int paused, const char* end_time_str) {
  draw_countdown(remaining, paused);
  draw_progress_bar(remaining, total, end_time_str);
  refresh();
}

static void handle_time_tick(int *remaining_seconds, time_t *next_tick, time_t *end_timestamp, int paused) {
